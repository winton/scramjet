...
DataStream$1.prototype.accumulate = async function accumulate(func, into) {
    return new Promise((res, rej) => {
        ...
        this.tap().pipe(new scramjetCore.PromiseTransformStream({ // TODO:
...
DataStream$1.prototype.reduceNow = function reduceNow(func, into) {
    ...
    if (into instanceof events.EventEmitter) {
...
class StreamError extends Error {
    ...
    constructor(cause, stream$$1, code = "GENERAL", chunk = null) {
        code = cause.code || code;
        stream$$1 = cause.stream || stream$$1;
        ...
        Object.defineProperty(this, "stack", {
            get: function () {
                return combineStack(
                    ...
                    "  caused by:",
                    cause.stack,
                    `  --- raised in ${stream$$1.name} constructed ---`,
                    stream$$1.constructed
                );
            }
        });
...
const pipeIfTarget = (stream$$1, target) => (target ? stream$$1.pipe(target) : stream$$1);
 ...
 * @return {DataStream}
 */
DataStream$1.from = function(stream$$1, options, ...args) {
    const target = options instanceof this && options;

    if (stream$$1 instanceof this) {
        return target ? stream$$1.pipe(target) : stream$$1;
    }

    if (stream$$1 instanceof stream.Readable || (
        typeof stream$$1.readable === "boolean" &&
        typeof stream$$1.pipe === "function" &&
        typeof stream$$1.on === "function"
    )) {
        const out = target || new this(
            Object.assign(
                {},
                options,
                { referrer: stream$$1 instanceof DataStream$1 ? stream$$1 : null }
            )
        );

        stream$$1.pipe(out);
        stream$$1.on("error", e => out.raise(e));
        return out;
    }

    if (stream$$1 instanceof GeneratorFunction || stream$$1 instanceof AsyncGeneratorFunction) {
        const iterator = stream$$1(...args);
    ...
    if (Array.isArray(stream$$1))
        return pipeIfTarget(this.fromArray(stream$$1, options), target);

    const iter = stream$$1[Symbol.iterator] || stream$$1[Symbol.asyncIterator];
    if (iter)
        return pipeIfTarget(this.fromIterator(iter(), options), target);

    if (typeof stream$$1 === "function") {
 ...
 */
DataStream$1.prototype.JSONStringify = function JSONStringify(eol = os.EOL) {
    if (!eol)
        eol = "";
...
DataStream$1.prototype.keep = function keep(count = -1) {
    ...
    this.pipe(this.buffer = new rereadableStream.ReReadable({ length: count, objectMode: true }));
...
DataStream$1.prototype.reduce = function(func, into) {
    ...
    return this.tap().pipe(new scramjetCore.PromiseTransformStream({
        promiseTransform: (chunk) => {
 ...
 */
MultiStream.prototype.add =  function add(stream$$1) {

    if (stream$$1) {
        this.streams.push(stream$$1);
        this.setMaxListeners(this.streams.length + events.EventEmitter.defaultMaxListeners);
        this.emit("add", stream$$1, this.streams.length - 1);
        stream$$1.on("end", () => this.remove(stream$$1));
    }
...
class StreamWorker {
    ...
    async spawn() {
        return this.resolving || (this.resolving = new Promise((res, rej) => {
            ...
            this._child = child_process.fork(__dirname + "/stream-child", [this.ip]);
     ...
     */
    static async fork(count = os__default.cpus().length) {
 ...
 */
MultiStream.prototype.cluster = function cluster(clusterFunc, {plugins = [], threads = os.cpus().length * 2, StreamClass, createOptions = {}}) {
    const out = new this.constructor();

    StreamWorker.fork(threads);

    this.each(
        (stream$$1) => StreamWorker
            .fork(1)
            .then(
                ([worker]) => out.add(worker.delegate(stream$$1, clusterFunc, plugins)
...
MultiStream.prototype.each =  function each(aFunc, rFunc) {
    return Promise.all(
        ...
        () => {
            this.on(
                "add",
                (stream$$1) => Promise.resolve(stream$$1).then(aFunc)
            );

            if (rFunc)
                this.on(
                    "remove",
                    (stream$$1) => Promise.resolve(stream$$1).then(rFunc)
                );
...
MultiStream.prototype.filter =  function filter(func) {
    return Promise.all(
        ...
        (streams) => {
            ...
            this.on(
                "add",
                (stream$$1) => Promise.resolve(stream$$1)
                    .then(func)
...
const mergesortStream = (multi, passedComparator, bufferLength, Clazz) => {
    ...
    const onceTouchedStream = (stream$$1) => {
        return Promise.race([
            new Promise((res) => stream$$1.on("readable", res)),
            endIndex.get(stream$$1)
        ]);
    };

    const getMoreItemsForEntry = (stream$$1, arr) => {
        while (arr.length < bufferLength) {
            let haveMore = stream$$1.read();
        ...
        if (arr.length || !readIndex.has(stream$$1))
            return Promise.resolve(arr);

        return onceTouchedStream(stream$$1)
            .then(
                () => getMoreItemsForEntry(stream$$1, arr),
                () => Promise.resolve(arr)
            );
    };
...
MultiStream.prototype.mux =  function mux(cmp, Clazz) {
    ...
    if (!cmp) {
        ...
        const unpipeStream = (stream$$1) => {
            if (stream$$1) stream$$1.unpipe(this[OUT]);
            this[OUT].setMaxListeners(this.streams.length);
        };

        const pipeStream = (stream$$1) => {
            this[OUT].setMaxListeners(this.streams.length);
            stream$$1.pipe(this[OUT], {end: false});
        };
 ...
 */
MultiStream.prototype.remove =  function remove(stream$$1) {

    const strIndex = this.streams.indexOf(stream$$1);
    if (strIndex >= 0) {
        this.setMaxListeners(this.streams.length + events.EventEmitter.defaultMaxListeners);
        this.streams.splice(strIndex, 1);
        this.emit("remove", stream$$1, strIndex);
    }
 ...
 */
MultiStream.prototype.route = function route(policy, count = os.cpus().length) {
...
DataStream$1.prototype.separate = function separate(affinity, createOptions, CreateClass) {
    ...
    this.pipe(
        new this.constructor({
            ...
            .on("end", () => {
                ret.streams.forEach(stream$$1 => stream$$1.end());
            })
            .resume()
    );
...
DataStream$1.prototype.tee = function(func) {
    if (func instanceof stream.Writable)
 ...
 */
class DataStream$1 extends scramjetCore.ScramjetStream {
